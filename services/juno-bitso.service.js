...
static async getUserClabeBalance(clabe) {
    try {
        // 1. Verificar que la CLABE existe y está activa
        const clabeDetails = await this.retrieveJunoClabeDetails(clabe);

        if (!clabeDetails.success) {
            return {
                success: false,
                error: { message: `CLABE ${clabe} not found in Juno account` },
            };
        }

        // 2. Obtener el balance REAL de MXNB de la cuenta principal de Juno
        const junoBalance = await this.retrieveBalance();
        console.log(`Juno balance retrieved for CLABE ${clabe}:`, junoBalance);
		console.log(junoBalance.payload);

        if (!junoBalance.success) {
            return {
                success: false,
                error: { message: 'Could not retrieve MXNB balance from Juno' },
            };
        }

        // 🔥 ARREGLO: Buscar MXNB con diferentes variaciones que puede devolver Juno
        const possibleAssets = ['mxnb', 'mxnbj', 'MXNB', 'MXNBJ'];
        let mxnbBalance = null;

        for (const assetName of possibleAssets) {
            mxnbBalance = junoBalance.payload.balances.find(b =>
                b.asset.toLowerCase() === assetName.toLowerCase()
            );
            if (mxnbBalance) {
                console.log(`✅ Found balance with asset: "${mxnbBalance.asset}"`);
                break;
            }
        }

        // Si no encuentra ninguna variación de MXNB, usar el primer balance disponible
        if (!mxnbBalance && junoBalance.payload.balances.length > 0) {
            mxnbBalance = junoBalance.payload.balances[0];
            console.log(`⚠️ Using first available balance with asset: "${mxnbBalance.asset}"`);
        }

        if (!mxnbBalance) {
            console.log('❌ Available balances:', junoBalance.payload.balances.map(b => b.asset));
            return {
                success: false,
                error: { message: 'No MXNB or similar balance found in Juno account' },
            };
        }

        console.log(`✅ Using balance:`, {
            asset: mxnbBalance.asset,
            available: mxnbBalance.available,
            total: mxnbBalance.total
        });
        ...
        return {
            ...
            payload: {
                clabe: clabe,
                available: mxnbBalance.available.toString(),
                total: mxnbBalance.total.toString(),
                locked: (mxnbBalance.on_hold || mxnbBalance.locked || 0).toString(),
                currency: mxnbBalance.asset.toUpperCase(), // Usar el asset real que devolvió Juno
                status: clabeDetails.payload.status || 'ENABLED',
                source: 'juno_mxnb_real_time',
            },
        };

    } catch (error) {
        console.error(`Error getting REAL Juno balance for CLABE ${clabe}:`, error);
        return {
            success: false,
            error: { message: `Juno API error: ${error.message}` },
        };
    }
}

	/**
	 * Validates if a user has sufficient REAL MXNB balance in Juno for a transaction.
	 * @param {string} clabe - The user's CLABE
	 * @param {number|string} requiredAmount - The amount needed
	 * @returns {Promise<{success: boolean, hasEnoughBalance: boolean, currentBalance: string, error?: any}>}
	 */
	static async validateJunoBalance(clabe, requiredAmount) {
		try {
			const balanceResponse = await this.getUserClabeBalance(clabe);
			...
			const availableBalance = parseFloat(balanceResponse.payload.available);
			const required = parseFloat(requiredAmount);

			return {
				success: true,
				hasEnoughBalance: availableBalance >= required,
				currentBalance: balanceResponse.payload.available,
				totalBalance: balanceResponse.payload.total,
				lockedBalance: balanceResponse.payload.locked,
			};

		} catch(error) {
			console.error(`Error validating Juno balance for CLABE ${ clabe }:`, error);
			return {
				success: false,
				hasEnoughBalance: false,
				currentBalance: '0.00',
