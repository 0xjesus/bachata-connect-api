...
class UserController extends PrimateController {
    ...
    static async getStats(req, res) {
        try {
            ...
            const eventsCreated = await primate.prisma.event.findMany({ where: { hostId: userId }, include: { _count: { select: { participants: true, }, }, }, });
            const participations = await primate.prisma.participation.findMany({ where: { userId }, include: { event: { select: { id: true, title: true, status: true, }, }, }, });
    ...
    static async avatar(req, res) {
        ...
        const { size = 128, width = 128, height = 128, bold = true, background = 'FFFFFF', color = '000000', fontSize = 64, border = 2, chars = 2, mode = 'light', format = 'png', } = req.query;
    ...
    static async updateAvatar(req, res) {
        try {
            ...
            const avatarAttachment = await UploadService.createAttachment({ buffer: avatarBuffer, size: avatarBuffer.length, originalname: `avatar-${file.originalname}`, mimetype: file.mimetype, }, { metas: { type: 'avatar', originalAttachment: originalAttachment.id } });
            const updatedUser = await UserService.update(idUser, { metas: { ...currentUser.metas, idAvatar: avatarAttachment.id, idAvatarOriginal: originalAttachment.id, }, });
            return res.respond({ data: { user: updatedUser, avatar: avatarAttachment, original: originalAttachment }, message: 'User avatar updated successfully', });
        } catch (e) {
